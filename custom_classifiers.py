...
class PerceptronClassifier():
    def __init__(self, learning_rate=0.001, max_iter=200, verbose=0):
...
def generate_xor_problem(cluster_size=1000):
...
def plot_binary_2d_dataset(trainX, testX, trainY, testY, title=""):
    ...
    if len(trainX) > 0 and trainX.shape[1] > 2:
        ...
        merged_2d = TSNE(n_components=2, verbose=0).fit_transform(merged)
...
def load_binary_xor_dataset(test_size=0.25, cluster_size=1000):
    dataX, dataY = generate_xor_problem(cluster_size=cluster_size)
