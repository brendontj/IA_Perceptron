...
if __name__ == "__main__":
    ...
    for DATASET_NAME in DATASET_LIST:
        ...
        p_clf = PerceptronClassifier(learning_rate=1.0, max_iter=3000, verbose=False)
        p_clf.fit(trainX, trainY)
        y_pred = p_clf.predict(testX)
        
        clf = MLPClassifier(solver='lbfgs', hidden_layer_sizes=(20, 20, 20), learning_rate_init=1.0, learning_rate='constant', activation= 'logistic', max_iter=400, verbose=False)
        clf.fit(trainX, trainY)
        y_pred_mlp = clf.predict_proba(testX)
        y_aux = np.argmax(y_pred_mlp, 1)
        print("Accuracy MLP classifier {} | Accuracy Perceptron classifier {}".format(accuracy_score(testY, y_aux), p_clf.score(testX,testY)))

        # Código utilizado para gerar dados
    ...
    #                 "type_classifier": "PerceptronClassifier",
